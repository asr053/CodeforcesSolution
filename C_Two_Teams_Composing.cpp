...
void solve(){
    int n;cin>>n;
    int x;unordered_map<int,int>p;
    for(int i=0;i<n;i++){cin>>x;p[x]++;}
    int value = INT_MIN,count=0;
    for(auto it:p){count++;if(it.second>value)value=it.second;}
    if(value-1>=count)cout<<count<<endl;
    else
    cout<<min(value,count-1)<<endl;
}
